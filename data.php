...
<head>
	    ...
	    ?>			  
                 <tr>     
					...
					<button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#staticBackdrop" data-bs-snm="<?php echo $row['stud_name']; ?>" data-bs-cr="<?php echo $row['course']; ?>" data-bs-cn="<?php echo $row['contact'];?>" data-bs-id="<?php echo $row['id']; ?>">Update</button>     
   ...
   if(isset($_POST['txtid']))
	{
		...
		$cr = $_POST['txtcr1'];
		$cn = $_POST['txtcn1'];
		$sql="UPDATE `stud_info` SET `stud name`='$snm',`course`='$cr',`contact`='$cn' WHERE 1 WHERE `id`='id'";
		$res=mysqli_query ($con,$sql);
